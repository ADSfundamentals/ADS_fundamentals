...
\begin{frame}{Prerequisite Knowledge}
  ...
  After completeing the preperatory exercises, you should already be familiar with the following ideas.
...
\begin{frame}[fragile, allowframebreaks]{Some Data}

  Let's create a few data frames to work with in the next few slides.
...
\begin{frame}[fragile]{Sorting \& Ordering}
  ...
  To sort the rows of a data frame according to the order of one of its columns, the \src{dplyr::arrange()} works best.
...
\begin{frame}[fragile]{Why are pipes useful?}
  ...
  <<>>=
  boys <-
      readRDS(here::here("data", "boys.rds") |>
...
\begin{frame}[fragile]{What do pipes do?}
  ...
  \vc
  \begin{itemize}
  \item \src{h(g(f(x)))} becomes \src{x} \pipe \src{f()} \pipe \src{g()} \pipe \src{h()}
  \end{itemize}
...
\begin{frame}[fragile]{The Role of \src{_} in a Pipeline}

  In the expression \src{a} \pipe \src{f(arg1, arg2, arg3)}, \src{a} will be "piped into" \src{f()} as \src{arg1}.
  ...
  \vc
  \begin{itemize}
  \item We can change this behavior with the \src{_} symbol.
    \vc
  \item The \src{_} symbol acts as a placeholder for the data in a pipeline.
  \end{itemize}
...
\begin{frame}[fragile]{Exposition Pipe: \expipe}
  ...
  The \pkg{magrittr} package provides several different flavors of pipe. The \emph{exposition pipe}, \expipe, is a
...
\begin{frame}{Some Programming Tips}
  ...
  \item Use comments to clarify what you are doing.
    \vc
  \item When working with functions in RStudio, use the TAB key to quickly access the documentation of the function's
    arguments.
    \vc
