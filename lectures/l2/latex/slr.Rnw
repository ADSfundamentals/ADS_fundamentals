...
\section{Defining Simple Linear Regression}

\watermarkoff %--------------------------------------------------------------------------------------------------------%

\begin{frame}[fragile]{Visualizations of Simple Linear Regression}
  
  \begin{columns}
    \begin{column}{0.5\textwidth}
      
      Earlier, we used \src{geom\_smooth()} to visualize the \emph{best fit line} from a simple linear regression model.
...
\begin{frame}{Estimating the Regression Coefficients}
  ...
  The purpose of regression analysis is to use a sample of $N$ observed $\{Y_n, X_n\}$ pairs to find the best fit line
  defined by $\hat{\beta}_0$ and $\hat{\beta}_1$.
  \vb
  \begin{itemize}
  \item The most popular method of finding the best fit line involves minimizing the sum of the squared residuals.
    \vb
...
\begin{frame}{Residuals as the Basis of Estimation}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      The $\hat{\varepsilon}_n$ are defined in terms of deviations between each observed $Y_n$ value and the
      corresponding $\hat{Y}_n$.
      \begin{align*}
...
\begin{frame}[fragile]{Least Squares Example}
  ...
  \begin{itemize}
  \item For every additional \$1000 in price, a car is expected to gain \Sexpr{b1} horsepower.
  \end{itemize}
...
\begin{frame}{Model Fit}
  ...
  \item The residual variance $\hat{\sigma}^2 = \textrm{Var}(\hat{\varepsilon})$ will be less than $\textrm{Var}(Y)$.
  \end{itemize}
...
\begin{frame}[shrink = 5]{Model Fit}
  ...
  \end{align*}
  Indicating that car price explains \Sexpr{r2 * 100}\% of the variability in horsepower.
...
\begin{frame}{Model Fit for Prediction}
  ...
  When assessing predictive performance, we will most often use the \emph{mean squared error} (MSE) as our criterion.
  \vb
  \begin{align*}
...
\begin{frame}{Interpreting MSE}
  ...
  \vx{-8}
  \begin{itemize}
  \item When using price as the only predictor of horsepower, we expect prediction errors with magnitudes of
    \Sexpr{rmse} horsepower.
  \end{itemize}
...
\begin{frame}{Information Criteria}
  ...
  We can use \emph{information criteria} to quickly compare \emph{non-nested} models while accounting for model
  complexity.\\
...
\section{Regression as Statistical Modeling}

%----------------------------------------------------------------------------------------------------------------------%

\begin{frame}{Linear Regression as a Statistical Model}
