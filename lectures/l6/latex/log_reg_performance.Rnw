...
\begin{frame}[fragile]{Optimized Confusion Matrix}
...
\begin{frame}[fragile]{Compare Thresholds}

  \begin{columns}
    \begin{column}{0.5\textwidth}

<<echo = FALSE, results = "asis">>=
cTab <- matrix(cMat$table, 2, 2,
               dimnames =  list(c("Died", "Survived"), c("Died", "Survived"))
               ) |> 
  xtable(caption ="Naive Threshold", digits = 0)
...
print(cTab, add.to.row = adds, include.colnames = FALSE)
@
...
<<echo = FALSE, results = "asis">>=
cTab2 <- matrix(cMat2$table, 2, 2,
               dimnames =  list(c("Died", "Survived"), c("Died", "Survived"))
               ) |> 
  xtable(caption ="Optimized Threshold", digits = 0)
...
print(cTab2, add.to.row = adds, include.colnames = FALSE)
@
...
<<echo = FALSE, results = "asis">>=
statMat <- rbind(
  c(cMat$overall[1], cMat$byClass[1:4]),
  c(cMat2$overall[1], cMat2$byClass[1:4])
)
dimnames(statMat) <- list(c("Naive", "Optimized"), c("Acc", "Sen", "Spe", "PPV", "NPV"))
...
\begin{frame}{Alternative Performance Measures}
  ...
  \item The CEE doesn't depend on the classification threshold
  \end{itemize}
...
\begin{frame}[fragile]{CEE Example}
...
Don't get too excited.
  \begin{itemize}
    \item We're using fitted values, not out-of-sample predictions.
    \item The larger model will always have a lower training-set CEE.
  \end{itemize}
