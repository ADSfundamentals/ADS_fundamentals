...
\begin{frame}{Components of the General Linear Model}
  ...
  \begin{itemize}
    \item $\eta$ is the \emph{systematic component} of the model (AKA, the \emph{linear predictor}).
    \item The normal distribution, $\mathrm{N}(\cdot, \cdot)$, is the model's \emph{random component}.
  \end{itemize}
...
\begin{frame}{Components of the General Linear Model}
  ...
  \begin{itemize}
  \item In the general linear model, we assume that this error variance follows a normal distribution.
  \end{itemize}
...
\begin{frame}{Components of the Generalized Linear Model}
  ...
  The random component in a GLM can be any distribution from the so-called \emph{exponential family}.
  \vc
  \begin{itemize}
...
\begin{frame}{Link Functions}
  ...
  \begin{enumerate}
    \item Linearize the association between $\mathbf{X}$ and $Y$.
      ...
      \end{itemize}
      \vc
    \item Allows GLMs for outcomes with restricted ranges without requiring any restrictions on the range of the
      $\{X_p\}$.
      \begin{itemize}
        \item In many cases, $\mu_Y$ has a limited range.
          ...
          \end{itemize}
        \item When correctly specified, $g(\mu_Y)$ can take any value on the real line.
      \end{itemize}
  \end{enumerate}
...
\begin{frame}[fragile]{Intuition of Link Functions}

  Suppose you uncover a burgeoning conspiracy to insight widespread civil unrest through a coordinated campaign of loud
  mobile phone conversations on public transit. 
  \vc
  \begin{itemize}
    \item If you want to predict the potential size of this terrorist cell, you could use a simple model of exponential
      growth.
  \end{itemize}
  \begin{align*}
    \mu = n_0 (1 + n_1)^t
  ...
  Where:
  \begin{itemize}
    \item $\mu$ is the group size at time $t$
    \item $n_0$ is the initial group size
    \item $n_1$ is the number of new members each conspirator recruits in a unit of time
  \end{itemize}
...
\begin{frame}[fragile]{Intuition of Link Functions}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \vc
      \begin{itemize}
        \item The group has five members when you start tracking them.
          \vc
        \item Every conspirator recruits two new members per week.
      \end{itemize}
      \vc
      Then, the number of aural assailants at Week $t$ is given by:
      \begin{align*}
        \mu = 5 (1 + 2)^t
      \end{align*}
...
ggplot(dat2, aes(t, y)) +
  ...
  ylab(expression(paste(mu, " = Conspirators"))) +
  xlab("Weeks") +
...
\begin{frame}[fragile]{Intuition of Link Functions}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      We can linearize the model by using the natural log as a link function.
      \begin{align*}
        \ln(\mu) &= \ln \left[ n_0 (1 + n_1)^t \right]\\
        &= \ln(n_0) + \ln(1 + n_1) t
      \end{align*}

      For our example problem, we get:
      \begin{align*}
        \ln(\mu) &= \ln \left[ 5 (1 + 2)^t \right]\\
        &= \ln(5) + \ln(3) t
      \end{align*}
...
<<echo = FALSE>>=
ggplot(dat2, aes(t, log(y))) +
  geom_line(color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  theme_minimal() +
  scale_x_continuous(expand = c(0, 0)) +
  ylab(expression(paste("g(", mu, ") = ", ln, "(Conspirators)"))) +
  xlab("Weeks") +
...
\begin{frame}[fragile]{Intuition of Link Functions}
  ...
  We can use the linearized model to estimate the $n_0$ and $n_1$ parameters with standard linear modeling techniques.
...
t <- 0:6
dat3 <- data.frame(t, mu = n0 * (1 + n1)^t)
xtable(dat3) |> print(include.rownames = FALSE)
@
...
<<>>=
# Estimate a log-linear model of group size:
out <- lm(log(mu) ~ t, data = dat3)

# Extract the estimated regression weights:
(beta <- coef(out))

# Convert the weights to a counts scale:
exp(beta)
@
...
\begin{frame}{Logistic Regression}
  ...
  \begin{itemize}
  \item To do so, we will use a very useful type of GLM: \emph{logistic regression}.
  \end{itemize}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \item The probability of passing must be between 0 and 1.
        \vc
      \item We care about the probability of passing, but we only observe absolute success or failure.
        \vc
        \begin{itemize}
        \item $Y \in \{1, 0\}$
        \end{itemize}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \item The link function ensures legal predicted values.
        \vc
      \item The sigmoidal curve implies fluctuation in the effectiveness of extra study time.
        \vc
        \begin{itemize}
        \item More study time is most beneficial for students with around \Sexpr{round(-beta[1] / beta[2], 2)} hours of
          study.
        \end{itemize}
      \end{itemize}
...
<<echo = FALSE>>=
utmb <- readRDS(here::here(dataDir, "utmb_finish_2017.rds"))
@

In 2017, \Sexpr{nrow(utmb)} people participated in the \emph{Ultra-Trail du Mont-Blanc}, but only
\Sexpr{round(100 * mean(utmb$Finish == "Yes"), 2)}\% finished the race.
...
\begin{frame}{Defining the Logistic Regression Model}
  ...
  \begin{itemize}
  \item The systematic component in our logistic regression model will be the binomial distribution.
  \end{itemize}
  \vb
...
\begin{frame}[fragile]{Link Function for Logistic Regression}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      Because $\pi$ is bounded by 0 and 1 and not linear related to $\mathbf{X}$, we cannot model it directly---we must
...
\begin{frame}{Fully Specified Logistic Regression Model}
  ...
  \end{align*}
  The fitted coefficients, $\{\hat{\beta}_0, \hat{\beta}_p\}$, are interpreted in units of \emph{log odds}.
...
\vx{-12}
\begin{itemize}
\item A student who does not study at all has \Sexpr{b0} log odds of passing the exam.
\item For each additional hour of study, a student's log odds of passing increase by \Sexpr{b1} units.
\end{itemize}
\vb
...
\begin{frame}{Interpretations}
  ...
  \item We can interpret $\hat{\beta}$ as we would in linear regression:
    \begin{itemize}
    \item A unit change in $X_p$ produces an expected change of $\hat{\beta}_p$ units in $\mathrm{logit}(\pi)$.
    \end{itemize}
    \vc
  \item After exponentiation, however, unit changes in $X_p$ imply multiplicative changes in $O_s = \pi / (1 - \pi)$.
    \begin{itemize}
    \item A unit change in $X_p$ results in multiplying $O_s$ by $e^{\hat{\beta}_p}$.
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Interpretations}
  ...
  \begin{itemize}
  \item A student who does not study is expected to pass the exam with odds of \Sexpr{b0e}.
  \item For each additional hour a student studies, their odds of passing increase by \Sexpr{b1e} \emph{times}.
    \begin{itemize}
    \item Odds of passing are \emph{multiplied} by \Sexpr{b1e} for each extra hour of study.
    \end{itemize}
  \end{itemize}
  ...
  \item $\hat{\beta_p} > 0$ = "Increased Success",  $e^{\hat{\beta}_p} > 1$ = "Increased Success"
  \end{itemize}
\end{frame}
...
@
  ...
  \vc
  \begin{itemize}
    \item The expected log-odds of males finishing the race are \Sexpr{results$b[2]} units higher than expected log-odds
      of females finishing the race.
  \end{itemize}
...
\begin{frame}[fragile, allowframebreaks]{Interpretation}
  ...
  The expected odds of males finishing the race are \Sexpr{oHat[2]} times the expected odds of females finishing the
  race.
...
\begin{frame}{Multiple Logistic Regression}
  ...
  \vc
  \begin{itemize}
  \item Including multiple predictor variables in the systematic component leads to \emph{multiple logistic regression}.
    \vc
  \item The relative differences between simple logistic regression and multiple logistic regression are the same as
    those between simple linear regression and multiple linear regression.
    \vc
    \begin{itemize}
    \item The only important complication is that the regression coefficients become partial effects.
    \end{itemize}
  \end{itemize}
...
@
  ...
  A passenger's gender significantly predicts their probability of survival, after controlling for their age and ticket class
...
\begin{frame}[fragile, allowframebreaks]{Model Comparison}
  ...
  We don't have an $R^2$ statistic for logistic regression models, so we need to use a \emph{likelihood ratio test} to
...
\begin{frame}{Predictions from Logistic Regression}
  ...
  Given a fitted logistic regression model, we can get predictions for new observations of $\{X_p\}$, $\{X_p'\}$.
  \vc
  \begin{itemize}
  \item Directly applying $\{\hat{\beta}_0, \hat{\beta}_p\}$ to $\{X_p'\}$ will produce predictions on the scale of
    $\eta$:
    \begin{align*}
      \hat{\eta}' = \hat{\beta}_0 + \sum_{p = 1}^P \hat{\beta}_p X_p'
    \end{align*}
  \item By applying the inverse link function, $g^{-1}(\cdot)$, to $\hat{\eta}'$, we get predicted success
    probabilities:
    \begin{align*}
      \hat{\pi}' = g^{-1}(\hat{\eta}')
    \end{align*}
...
\begin{frame}{Predictions from Logistic Regression}
  ...
  \begin{align*}
    \hat{\pi}' &= \frac{e^{\hat{\eta}'}}{1 + e^{\hat{\eta}'}} =
      \frac{\exp \left(\hat{\beta}_0 + \sum_{p = 1}^P \hat{\beta}_p X_p' \right) }
        {1 + \exp \left(\hat{\beta}_0 + \sum_{p = 1}^P \hat{\beta}_p X_p' \right) }
  \end{align*}
...
\begin{frame}{Classification with Logistic Regression}
  ...
  \vc
  \begin{itemize}
  \item By choosing a threshold on $\hat{\pi}'$, say $\hat{\pi}' = t$, we can classify the new observations as
...
\begin{frame}[allowframebreaks]{Classification Example}
  ...
  \begin{itemize}
  \item Because $\Sexpr{pi1} \Sexpr{c("<", ">")[g]} 0.5$ we would classify this passenger as having
    \Sexpr{c("survived", "died")[g]}.
  \end{itemize}
