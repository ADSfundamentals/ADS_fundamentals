...
\begin{frame}{Assumptions for Logical Inference}
  ...
  Given \{Assumptions, Estimates\} $\rightarrow$ Inference
...
\begin{frame}{Assumptions vs. Estimates}
  ...
  Model = Assumptions + Estimates

  \begin{align*}
    M &= \gamma_1 A + \gamma_2 E\\
    1 &= \gamma A + (1 - \gamma) E
  \end{align*}

  \begin{itemize}
    \item $\gamma = 1$: Because I said so
    \item $\gamma = 0$: Infinite model complexity
...
\begin{frame}{Treating Heteroscedasticity}
  ...
  \item Use a \emph{Heteroscedasticity Consistent} (HC) estimate of the asymptotic covariance matrix.
    \vc
    \begin{itemize}
    \item Robust standard errors, Huber-White standard errors, Sandwich estimators
