...
\begin{frame}{Exam Information}
  ...
  \vc
  \begin{itemize}
  \item Exam: Thursday 22 January
    \vc
  \item Resit: Monday 23 February
  \end{itemize}
  \vb
  Structure
  \vc
...
\begin{frame}{Simple Linear Regression}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \vx{-18}
      \begin{itemize}
      \item For any $X_n$, the corresponding $\hat{Y}_n$ represents the model-implied, conditional mean of $Y$.
      \end{itemize}
      \vb
      \only<2>{
...
\begin{frame}[allowframebreaks]{Assumptions}
  ...
  \item The errors are normally distributed.\label{normErr}
    \begin{itemize}
    \item \emph{Otherwise:} Small-sample inferences and some estimates are not justified.
    \end{itemize}
...
\begin{frame}[fragile]{Visualizing Categorical Moderation}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      p3 <- ggplot(data    = socSup, mapping = aes(x = tanSat, y = bdi, colour = factor(sex))) +
          theme_classic() +
          theme(text = element_text(family = "Courier", size = 16))
...
\begin{frame}{Prediction Example}
  ...
  \begin{align*}
    \hat{Y}_{LDL} = \Sexpr{b0} + \Sexpr{b1} X_{BP} + \Sexpr{b2} X_{gluc} + \Sexpr{b3} X_{BMI}
  \end{align*}
  \pause
  Suppose a new patient presents with $BP = \Sexpr{x1}$, $gluc = \Sexpr{x2}$, and $BMI = \Sexpr{x3}$. We can predict
  their $LDL$ score by:
  \begin{align*}
...
\begin{frame}{Interval Estimates Example}
  ...
  \vx{-16}
  \begin{itemize}
  \item We can be 95\% confident that the \underline{average \emph{LDL}} of patients with \emph{Glucose} = 89,
    \emph{BP} = 121, and \emph{BMI} = 30.6 will be somewhere between \Sexpr{confInt[1]} and \Sexpr{confInt[2]}.
    \vb
...
\begin{frame}[shrink = 5]{Model Fit}
  ...
  where
  \begin{align*}
    TSS = \sum_{n = 1}^N \left(Y_n - \bar{Y}\right)^2 = \textrm{Var}(Y)\times (N - 1)
  \end{align*}
  ...
  \begin{align*}
    R^2 = 1 - \frac{\Sexpr{as.integer(ssr)}}{\Sexpr{as.integer(sst)}} \approx \Sexpr{r2}
  \end{align*}
...
\begin{frame}{Information Criteria}
  ...
  We can use \emph{information criteria} to quickly compare \emph{non-nested} (or nested) models while accounting for
  model complexity.
...
\begin{frame}{Probabilities \& Odds}
  ...
  utmb <- readRDS(here::here("data", "utmb_finish_2017.rds"))
...
\begin{columns}
  \begin{column}{0.45\textwidth}
    ...
    \begin{alignat*}{2}
      P(C | M) &=
      \frac{\Sexpr{xTab["Male", "Yes"]}}{\Sexpr{xTab["Male", "Yes"]} + \Sexpr{xTab["Male", "No"]}} &&= \Sexpr{pCM}\\[8pt]
      P(C | F) &=
      \frac{\Sexpr{xTab["Female", "Yes"]}}{\Sexpr{xTab["Female", "Yes"]} + \Sexpr{xTab["Female", "No"]}} &&= \Sexpr{pCF}
    \end{alignat*}
...
\begin{frame}{The Logistic Regression Model}
  ...
  \end{align*}
  To convert fitted values, $\hat{\eta} = \hat{\beta}_0 + \sum_{p = 1}^P \hat{\beta}_p X_p$, from a logit scale to a
  probability scale, we apply the \emph{logistic} function:
  \begin{align*}
...
\begin{frame}[allowframebreaks]{Classification Example}
  ...
  Say we want to classify a new patient into either the ``high glucose'' group or the ``not high glucose'' group using
  the model fit above.
  \begin{itemize}
  ...
  \vc
  \begin{itemize}
  \item Because $\Sexpr{pi1} \Sexpr{c("<", ">")[g]} 0.5$ we would classify this patient into the
    ``\Sexpr{c("low", "high")[g]} glucose'' group.
  \end{itemize}
...
<<echo = FALSE, out.width = "65%">>=
predict(out1, type = "response") |> roc(yTrue, predictor = _) |> plot()
@
