...
<<>>=
(maxCD  <- cooks.distance(glmFit) |> which.max())
(maxDFB <- dfbetas(glmFit)[ , "fare"] |> abs() |> which.max())
...
# View the three most influential cases:
titanic0[mostInf, ]

# Refit the model excluding the three most influential cases:
titanic3 <- titanic[-mostInf, ]
glmFit3 <- update(glmFit, data = titanic3)
@
